...
if __name__ == '__main__':
    ...
    parser.add_argument('--code_prefix_len', type=int, help='prefix of codes used to group items into buckets', default=0)
    ...
    if args.code_prefix_len == 0:
        code_prefix_len = quantizer.num_codebooks
    else:
        code_prefix_len = args.code_prefix_len

    buckets = bucket(code_prefix_len, quantizer.Ks, None, None, codes)
    norm_buckets = bucket(code_prefix_len, 256, None, None, codes)

    X_norms = np.linalg.norm(X, axis=1)

    max_norms = np.array([max(X_norms[bucket]) for bucket in norm_buckets], dtype=np.float32)
    mean_norms = np.array([sum(X_norms[bucket]) / len for bucket in norm_buckets], dtype=np.float32)

    sort_mapping = np.argsort(max_norms)
    max_norms = max_norms[sort_mapping]
    mean_norms = mean_norms[sort_mapping]
