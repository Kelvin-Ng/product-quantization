...
class NormSphericalPQ(object):
    def __init__(self, n_percentile, Ks, true_norm=False, verbose=True, method='kmeans', recover='quantize'):
    ...
    def class_message(self):
        return "NormPQ, percentiles: {}".format(self.n_percentile)
    ...
    def fit(self, vecs, iter):
        ...
        self.codewords, _ = kmeans2(normalized_vecs, self.Ks, iter=1, minit='points')
        _, self.codewords = normalize(self.codewords)
        for i in range(iter-1):
            self.codewords, _ = increase_kmeans(normalized_vecs, self.codewords, iter=1)
            _, self.codewords = normalize(self.codewords)
