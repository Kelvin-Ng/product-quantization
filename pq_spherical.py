...
from pq_residual_iteratively import increase_kmeans
import math


class SphericalPQ(object):
    ...
    def class_message(self):
        return "Subspace Spherical PQ, M: {}, Ks : {}, code_dtype: {}".format(self.M, self.Ks, self.code_dtype)
    ...
    def fit(self, vecs, iter):
        ...
        for m in range(self.M):
            ...
            self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]], _ = kmeans2(vecs_sub, self.Ks, iter=1, minit='points')
            _, self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]] = normalize(self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]])
            self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]] /= math.sqrt(self.M)
            for i in range(iter-1):
                self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]], _ = increase_kmeans(vecs_sub, self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]], iter=1)
                _, self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]] = normalize(self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]])
                self.codewords[m, :, :self.Ds[m+1] - self.Ds[m]] /= math.sqrt(self.M)
