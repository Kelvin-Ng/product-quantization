...
if __name__ == '__main__':
    ...
    norm_buckets = bucket(1, 256, None, None, codes)

    X_norms = np.linalg.norm(X, axis=1)

    max_norms = np.array([max(X_norms[bucket]) if len(bucket) > 0 else 0 for bucket in norm_buckets], dtype=np.float32)
    mean_norms = np.array([np.mean(X_norms[bucket]) if len(bucket) > 0 else 0 for bucket in norm_buckets], dtype=np.float32)
    norm_bucket_sizes = np.array([len(bucket) for bucket in norm_buckets], dtype=np.intc)

    with open(args.save_dir + '/' + args.dataset + args.result_suffix + '_max_norms', 'wb') as f:
        max_norms.tofile(f)

    with open(args.save_dir + '/' + args.dataset + args.result_suffix + '_mean_norms', 'wb') as f:
        mean_norms.tofile(f)

    with open(args.save_dir + '/' + args.dataset + args.result_suffix + '_norm_bucket_sizes', 'wb') as f:
        norm_bucket_sizes.tofile(f)

    buckets_write(args.save_dir + '/' + args.dataset + args.result_suffix + '_buckets', buckets, buckets_residue)
